...
Cluster::Cluster()
{
	...
	flag_leafCluster = false;
	flag_activeCluster = false;
	flag_inheritanceCluster = false;

	index = -1;
	distanceSuperParent = 0.0;
	num_stable_children = 0;
...
void Cluster::computeAQQHaptic()
{
	...
	for(vector<DuplicatedNode>::const_iterator hni = haptic_node_list.begin(); hni != haptic_node_list.end(); ++hni)
	{
		q  = hni->coordinate - original_center;
		a_qq_ += hni->mass*q*q.transpose();
	}
	for(vector<DuplicatedNode>::const_iterator gni = ghost_node_list.begin(); gni != ghost_node_list.end(); ++gni)
	{
		q  = gni->coordinate - original_center;
		a_qq_ += gni->mass*q*q.transpose();
	}
	a_qq = a_qq_.inverse();
}
