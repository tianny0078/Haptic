...
public:
	typedef enum {UNDEFINED, SHAPE_MATCHING, VELOCITY_MATCHING, PAIR_MATCHING, SINGLE_GRID,HIERARCHY,NEW_HIERARCHY_SHAPE_MATCHING, HIERARCHY_SHAPE_MATCHING,ADAPTIVE_HIERARCHY_SHAPE_MATCHING, ADAPTIVE_SHAPE_MATCHING, MULTIPLE_VELOCITY_MATCHING, SIMULATION_NETWORKING, SIMULATION_MOBILE} Simulator;
...
public:
	...
	bool simulateNextStep4NewHierarchyShapeMatching();
	bool simulateNextStep4AdaptiveHierarchyShapeMatching();
	bool simulateNextStep4AdaptiveShapeMatching();
	//
	...
	//newly tool
	Cluster * findLeafParent(Cluster * p);
	DuplicatedNode * findLeafNode4Children(DuplicatedNode * p, int idx_node);
	bool linkNode4LeafChildren(Cluster *p);
	bool linkNode4LeafParent(Cluster *p, Vector3d force_gravity, Vector3d force_wind);
	bool linkNode4LeafChildren(Cluster *p, Vector3d force_gravity, Vector3d force_wind);
	...
	void generatePerVoxCluster4Level(VoxMesh* &vm, VoxMesh* &parent_mesh, int levelIndex);
	...
	void linkNeighbor4Cluster(VoxMesh * vm);
	...
	//haptic
	void setHapticNode(int idx_triangle);
	void setHapticNode(int idx_triangle, Vector3d coordinate);
	void setGoalPos4HapticNode(Vector3d goalposition);
	bool getForce4HapticNode(Vector3d & force);
	bool getPos4HapticNode(Vector3d & pos);
...
public:
	...
	//adaptive threshold for energy
	double threshold_energy;
	int threshold_leaf_level;
	double runtime;
	int peNum;
