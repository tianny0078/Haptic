...
struct Cluster
{
	...
	vector<Cluster *> neighbor_list;
	//vector<int>leaf_levelIdx_list;
	//vector<int>leaf_clusterIdx_list;
	...
	//new for adaptive
	bool flag_leafCluster;
	bool flag_activeCluster;
	bool flag_inheritanceCluster;
	int index;
	double distanceSuperParent;
	int num_stable_children;

	//new for haptic
	vector<DuplicatedNode> haptic_node_list;
	vector<DuplicatedNode> ghost_node_list;
};
...
struct VoxMesh
{
	...
	vector<Vox>   vox_list;
	//for haptic
	vector<Node> haptic_node_list;
	vector<Node> ghost_node_list;
	...
	//vector<Node*> active_node_list;										// for more than one active for haptic (external force)
	...
	Node* active_node[2];													// the node where the external force applies to, at most 2
	...
	//adaptive
	vector<Cluster *> cluster_adaptive_list;
